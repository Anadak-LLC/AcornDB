...
{
    /// <summary>
    /// Crash/restart and corruption-detection tests for BPlusTreeTrunk.
    ///
    /// Categories:
    ...
    /// </summary>
    public class BPlusTreeCrashRestartTests : IDisposable
    {
        private readonly string _testDir;
        private readonly BPlusTreeOptions _defaultOptions;

        public BPlusTreeCrashRestartTests()
        {
            _testDir = Path.Combine(Path.GetTempPath(), $"acorndb_crash_{Guid.NewGuid()}");
            Directory.CreateDirectory(_testDir);
            _defaultOptions = new BPlusTreeOptions { PageSize = 4096, MaxCachePages = 64 };
        }
        ...
        public void WriteN_Dispose_Reopen_ReadAllBack()
        {
            ...
            using (var trunk = new BPlusTreeTrunk<string>(customPath: dir, options: _defaultOptions))
            {
        ...
        public void WAL_TruncatedUncommittedTail_DiscardedOnRecovery()
        {
            ...
            var walPath = Path.Combine(dir, "bplustree.wal");
            var dbPath = Path.Combine(dir, "bplustree.db");
        ...
        public void PageCRC_FlippedByte_DetectedOnRead()
        {
            ...
            var optionsWithCrc = new BPlusTreeOptions
            {
                PageSize = pageSize,
                MaxCachePages = 64,
            ...
            {
                using var trunk = new BPlusTreeTrunk<string>(customPath: dir, options: optionsWithCrc);
        ...
        public void PageSizeMismatch_DetectedOnOpen()
        {
            ...
            var wrongOptions = new BPlusTreeOptions { PageSize = 8192, MaxCachePages = 64 };

            var ex = Assert.Throws<InvalidDataException>(() =>
            {
                using var trunk = new BPlusTreeTrunk<string>(customPath: dir, options: wrongOptions);
            });
        ...
        public void MultiplePagesCorrupted_CorruptionDetected()
        {
            ...
            Assert.Throws<InvalidDataException>(() =>
            {
                using var trunk = new BPlusTreeTrunk<string>(customPath: dir, options: new BPlusTreeOptions
                {
                    PageSize = pageSize,
                    MaxCachePages = 64,
                    ValidateChecksumsOnRead = true
